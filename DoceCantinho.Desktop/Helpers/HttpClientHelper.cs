// =============================================================================
// SenacDoces.Desktop - Helpers/HttpClientHelper.cs
// =============================================================================
...
namespace DoceCantinho.Desktop.Helpers
{
    /// <summary>
    ...
    public sealed class HttpClientHelper
    {
        ...
        public async Task<(bool IsAvailable, string ErrorMessage)> PingApiAsync()
        {
            if (_client.BaseAddress == null)
            {
                return (false, "URL da API não configurada. Verifique o launchSettings.json " +
                               "do projeto DoceCantinho.API ou o appsettings.json do Desktop.");
            }

            try
            {
                // Faz uma requisição HEAD simples para verificar conectividade
                // GET /api/doces é público e leve o suficiente para um ping
                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
                var response = await _client.GetAsync("/api/doces", cts.Token);
        ...
        /// Uso: var doces = await http.GetAsync&lt;List&lt;DoceResponseDto&gt;&gt;("/api/doces");
        /// </summary>
        /// <typeparam name="T">Tipo de retorno esperado</typeparam>
        /// <param name="endpoint">Caminho do endpoint (ex: "/api/doces")</param>
        ...
        /// Uso: var doce = await http.PostAsync&lt;DoceResponseDto&gt;("/api/doces", createDto);
        /// </summary>
        ...
        private string CategorizeConnectionError(Exception ex, string endpoint)
        {
            ...
            if (ex is HttpRequestException httpEx)
            {
                    ...
                    msg.Contains("no connection could be made"))
                {
                    ...
                    return $"❌ A API não está em execução.\n\n" +
                           ...
                           $"Verifique se o projeto DoceCantinho.API está rodando no Visual Studio.";
                }
            ...
            if (ex is UriFormatException or InvalidOperationException)
            {
                return "⚠ URL da API inválida.\n\n" +
                       ...
                       "ou o launchSettings.json do projeto DoceCantinho.API.";
            }
